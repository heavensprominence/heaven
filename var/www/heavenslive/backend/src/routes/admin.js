...
router.get('/public-stats', async (req, res) => {
    ...
    let revenueVal=0,ordersVal=0,treasuryVal=0;
    try{const rv=await pool.query("SELECT COALESCE(SUM(amount_cents),0) as total FROM purchases WHERE status = 'paid'");revenueVal=parseInt(rv.rows[0].total)||0}catch(e){}
    try{const ov=await pool.query("SELECT COALESCE(SUM(amount_usd),0) as total FROM orders");ordersVal=parseInt(ov.rows[0].total)||0}catch(e){}
    try{const tv=await pool.query("SELECT COALESCE(SUM(CASE WHEN action = 'mint' THEN amount_cents WHEN action = 'burn' THEN -amount_cents WHEN action = 'distribute' THEN -amount_cents ELSE 0 END),0) as balance FROM treasury_ledger");treasuryVal=parseInt(tv.rows[0].balance)||0}catch(e){}
        ...
        res.json({
            ...
            total_revenue_cents: revenueVal + ordersVal,
            total_treasury_cents: treasuryVal,
...
router.post('/mint', verifyToken, requireAdmin, async (req, res) => {
    try {
        const { amount_cents, reason, email } = req.body;
        if (!amount_cents || amount_cents <= 0) return res.status(400).json({ error: 'Amount required' });
        
        if (email) {
            // Mint to specific user
            const user = await pool.query('SELECT id FROM users WHERE email = $1', [email]);
            if (user.rows.length === 0) return res.status(404).json({ error: 'User not found' });
            const result = await MockMinting.mint(user.rows[0].id, amount_cents, reason || 'Admin mint', req.userId);
...
router.post('/mint-bak', verifyToken, requireAdmin, async (req, res) => {
...
router.post('/burn', verifyToken, requireAdmin, async (req, res) => {
    try {
        ...
        if (email) {
            const user = await pool.query('SELECT id FROM users WHERE email = $1', [email]);
            if (user.rows.length === 0) return res.status(404).json({ error: 'User not found' });
            const result = await MockMinting.burn(user.rows[0].id, amount_cents, reason || 'Admin burn', req.userId);
            return res.json({ success: true, ...result });
        }
        
        // Burn from treasury
        await pool.query('INSERT INTO treasury_ledger (amount_cents, reason, action, admin_id, title) VALUES ($1, $2, $3, $4, $5)', [amount_cents, reason || 'Admin burn', 'burn', req.userId, 'Burned from treasury']);
        res.json({ success: true, message: 'Burned from treasury' });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/burn-bak', verifyToken, requireAdmin, async (req, res) => {
  const { userId, amountCents, reason } = req.body;
    ...
    try {
        ...
        try {
            ...
            if (u.rows[0]) {
                ...
                await sendLoanApproval(u.rows[0].email, u.rows[0].full_name || 'Friend', amountDisplay, finalCurrency, rateDisplay);
            }
...
router.get('/loans/pending', verifyToken, requireAdmin, async (req, res) => {
    try {
        const result = await pool.query(
            "SELECT lr.*, u.email, u.full_name FROM loan_requests lr JOIN users u ON lr.user_id = u.id WHERE lr.status = 'pending' ORDER BY lr.created_at DESC"
        );
        res.json({ loans: result.rows });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// Approve loan with custom amount and interest rate
router.post('/loans/:id/approve', verifyToken, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { amount_cents, interest_rate_percent } = req.body;
        
        const amt = parseInt(amount_cents) || 0;
        const rate = parseFloat(interest_rate_percent) || 0;
...
// Get active loans (for admin overview)
router.get('/loans/active', verifyToken, requireAdmin, async (req, res) => {
    try {
        const result = await pool.query(
            "SELECT al.*, u.email, u.full_name FROM active_loans al JOIN users u ON al.user_id = u.id WHERE al.remaining_cents > 0 ORDER BY al.start_date DESC"
        );
        res.json({ loans: result.rows });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
